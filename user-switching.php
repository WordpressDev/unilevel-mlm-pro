...
class user_switching {
	...
	function personal_options( $user ) {
		...
		<tr>
			...
			<td><a href="<?php _e($this->switch_to_url( $user->ID )); ?>"><?php _e( 'Switch&nbsp;To', 'user_switching' ); ?></a></td>
		</tr>
		<?php
	}
	...
	function switch_on() {
		...
		if ( !is_admin_bar_showing() and $old_user = $this->get_old_user() ) {
			...
			_e('<p id="user_switching_switch_on"><a href="' . $url . '">' . $link . '</a></p>');
		}
	...
	function bp_menu() {
		...
		if ( !is_admin() and $old_user = $this->get_old_user() ) {
			...
			_e('<li id="bp-adminbar-userswitching-menu" style="background-image:none"><a href="' . $this->switch_back_url() . '">');

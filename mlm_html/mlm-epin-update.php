...
function mlmePinUpdatePage()
{
         ...
         if(isset($mlm_settings['ePin_activate'])&& $mlm_settings['ePin_activate']=='1')
         {
            ...
            if($wpdb->num_rows > 0)
            {
                ...
                if($payment_status=='1')
                {
                    echo '<div style="background:#FFCC99;padding:10px;">'.__('Your status is already to set to Paid in the system. You cannot activate your membership again with an ePin.','unilevel-mlm-pro').'</div>';
                }
                else if($payment_status=='2')
                {
                    echo '<div style="background:#FFCC99;padding:10px;">'.__('Your status is already set to Active in the system. You cannot activate your membership again with an ePin.','unilevel-mlm-pro').'</div>';
                }
            }
            else
             {
                ...
                if($wpdb->num_rows=='0')
                 {
                  echo '<div style="background:#FFCC99;padding:10px;">'.__('Not MLM User','unilevel-mlm-pro').'</div>';     
                 }
                 else 
                 {
				 $payment_status = $wpdb->get_var("select payment_status from {$table_prefix}mlm_users where user_id='{$user->ID}'");
				...
				else if($payment_status=='2')
        {
         ...
         {
         echo '<div style="background:#FFCC99;padding:10px;">'.__('Sorry. You are not allowed to access this page due to administrative permissions.','unilevel-mlm-pro').'</div>';
         }

...
function mlm_core_update_mlm_user_master()
{
	global $wpdb;
	$charset_collate = mlm_core_set_charset();
	$table_prefix = mlm_core_get_table_prefix();
	
 	$sql[] =    "ALTER TABLE   {$table_prefix}mlm_users modify  payment_status ENUM( '1','0','2' ) NOT NULL DEFAULT '0' COMMENT '1 indicate paid and 0 indicate unpaid 2 Indicates special paid',{$charset_collate}";
	if(mysql_num_fields(mysql_query("select * from {$table_prefix}mlm_users"))==9)
	{
	mysql_query($sql[0])or  die(mysql_error().'ALTER');
...
function mlm_core_insert_into_country()
{
	...
	if(mysql_num_rows(mysql_query("select * from {$table_prefix}mlm_country"))<239)			
...
function mlm_core_insert_into_currency()
{
	...
	if(mysql_num_rows(mysql_query("select * from {$table_prefix}mlm_currency"))<168)			
	if(!$wpdb->query($insert));
...
function mlm_core_install_bonus()
{
	...
	$sql[] = "CREATE TABLE IF NOT EXISTS  {$table_prefix}mlm_bonus
			(
				...
				amount DOUBLE( 6,2 ) NOT NULL DEFAULT 0.00,
				payout_id int(11) NOT NULL DEFAULT '0',
				bonus_type int(2) NOT NULL DEFAULT '0' COMMENT 'Bonus Type: Regular Bonus=1, Royalty Bonus=2',
...
****************************************************************/
function mlm_core_install_payment_status()
{
	global $wpdb;
	$charset_collate = mlm_core_set_charset();
	$table_prefix = mlm_core_get_table_prefix();
	
	$sql[] = "CREATE TABLE IF NOT EXISTS {$table_prefix}mlm_payment_status
			(
...
Hierarchy Table 
****************************************************************/
function mlm_core_install_hierarchy()
{
	global $wpdb;
	$charset_collate = mlm_core_set_charset();
	$table_prefix = mlm_core_get_table_prefix();
	
	$sql[] = "CREATE TABLE IF NOT EXISTS {$table_prefix}mlm_hierarchy 
	(
...
function mlm_core_update_hierarchy()
{
	global $wpdb;
	$charset_collate = mlm_core_set_charset();
	$table_prefix = mlm_core_get_table_prefix();
	
 	$sql[] = "ALTER TABLE   {$table_prefix}mlm_hierarchy add  slab int(11) NOT NULL DEFAULT '0',{$charset_collate}";
	if(mysql_num_fields(mysql_query("select * from {$table_prefix}mlm_hierarchy"))==5)
	{
	mysql_query($sql[0])or  die(mysql_error().'ALTER');
...
function mlm_core_install_transaction()
{
	...
	$table_prefix = mlm_core_get_table_prefix();
	 ...
	 $sql[] = "CREATE TABLE IF NOT EXISTS  {$table_prefix}mlm_transaction
			(
				id BIGINT(20) NOT NULL  AUTO_INCREMENT PRIMARY KEY,
				user_id BIGINT(20) NOT NULL,
...
function mlm_core_install_withdrawal()
{
	...
	$sql[] = "CREATE TABLE IF NOT EXISTS  {$table_prefix}mlm_withdrawal
			(
				id BIGINT(20) NOT NULL  AUTO_INCREMENT PRIMARY KEY,
				user_id BIGINT(20) NOT NULL,
